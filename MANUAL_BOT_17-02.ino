...
int countLt = 0;
int countRt = 0;
int gripCount = 200; // Set by Left and Right control (Using arbitrary multiple of 5)
int controlR = 0;
...
float LeftY = 0;
float speeds = 0;
float cap = 1;  // SPEED CAP (127*cap) (LEFT JOYSTICK)
float angle = 0;
...
void readButtonStates()
{
  ...
  toggleOne = PS.readButton(PS2_CIRCLE);
  toggleAll = PS.readButton(PS2_SQUARE);
  ...
  else if ((gripRight == 0) && (prevGR == 1))
  {
    ...
    /*Serial.println(gripRight);
      Serial.println(prevGR);
    */
  ...
  else if (((toggleOne == 0) && (prevToggleOne == 1)))

  {
    select = gripCount % 5;
    if (gripState[select] == 1)
    {
    ...
    {
      ser[select].write(closePos[select]);
      gripState[select] = 1;
    }
    /*Serial.println(openGripper);
      Serial.println(prevOpen);*/
  }

  else if ((toggleAll == 0) && (prevToggleAll == 1))
  {
    if (allPress % 2 == 0)
    {
      for (int i = 0; i < 5; i++)
  ...
  prevFL = currentFL;
  prevFR = currentFR;
  prevToggleOne = toggleOne;
  prevToggleAll = toggleAll;
